  ...
  <footer data-testid="footer" className="relative bg-[#F5F5F0] text-[#1A3636] pt-20 pb-6 overflow-hidden border-t-2 border-[#1A3636]">
    <div className="max-w-7xl mx-auto px-5 md:px-10">
      <div className="grid grid-cols-2 md:grid-cols-5 gap-10 mb-16">
        ...
        <FooterCol title="Product" items={["Matchmaking", "Sessions", "Pro", "Install"]} />
        <FooterCol title="Company" items={["About", "Careers", "Press", "Contact"]} />
        <FooterCol title="Legal" items={["Privacy", "Terms", "Refunds", "Community"]} />
      </div>
...
const FooterCol = ({ title, items }) => (
  <div>
    ...
    <ul className="flex flex-col gap-3">
      {items.map((i) => (
        <li key={i}>
          <a href="#" className="text-sm text-[#1A3636] hover:text-[#F95738] transition-colors">{i}</a>
        </li>
      ))}
    </ul>
  </div>
